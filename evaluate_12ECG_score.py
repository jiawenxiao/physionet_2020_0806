...
def evaluate_12ECG_score(label_directory, output_directory):
    ...
    auroc, auprc = compute_auc(labels, scalar_outputs,classes)
...
def compute_auc(labels, outputs,classes):
    num_recordings, num_classes = np.shape(labels)
    ...
    auroc=auroc.reshape(1,-1)
    df=pd.DataFrame(auroc,columns=[classes])
    df.to_csv('auc.csv')
